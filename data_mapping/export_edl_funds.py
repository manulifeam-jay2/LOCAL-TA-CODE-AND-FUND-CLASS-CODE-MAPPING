...
query_sql = """
    WITH RankedDatesData AS (
        SELECT 
            ...
            effective_date,
            ROW_NUMBER() OVER (PARTITION BY fund_class_code_1, liability_portf_name, country_of_domicile_code, market_id ORDER BY effective_date DESC) AS rn
        FROM `hive_metastore`.`inv_dal_eod`.`cube_portfolios`
        WHERE country_of_domicile_code IN ('BB', 'HK', 'PH', 'TW', 'SG', 'MM', 'KR', 'CN', 'MY', 'TH', 'VN', 'IN', 'KH', 'ID', 'JP', 'LU')
        AND fund_class_code_1 <> ''
    )
    SELECT 
        market_id AS edl_market_id,
